...
def transform_test(image, label, logits=None, crop_size=(512, 512), scale_size=(0.8, 1.0), label_fill=255,
              augmentation=False):
    '''
    Only apply on the 3d image (one image not batch)
    '''
    ...
    #i = max(resized_size[0] - crop_size[0],0) // 2  # Starting row index
    #j = (resized_size[1] - crop_size[1]) // 2  # Starting column index
    ...
    #image = transform_f.crop(image, i, j, h, w)
    #label = transform_f.crop(label, i, j, h, w)
    # Fixed Cropping to the center
    i = (resized_size[0] - crop_size[0]) // 2
    j = (resized_size[1] - crop_size[1]) // 2
    image = transform_f.crop(image, i, j, crop_size[0], crop_size[1])
    label = transform_f.crop(label, i, j, crop_size[0], crop_size[1])
    if logits is not None:
        logits = transform_f.crop(logits, i, j,  crop_size[0], crop_size[1])
    ...
    label_tensor = (transform_f.to_tensor(label) * 255).long()
    label_tensor[label == 255] = -1  # incalid pixels are re-mapping to index -1
    if logits is not None:
        logits = transform_f.to_tensor(logits)

    # Apply (ImageNet) normalization
    # image = transform_f.normalize(image, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    image_tensor = transform_f.normalize(image_tensor, mean=[0.5], std=[0.299])
    if logits is not None:
        return image, label,image_tensor,label_tensor, logits
    else:
        return image, label,image_tensor,label_tensor
